...
namespace bitmap {
  constexpr int base_chars = 96;  // 96 printable ASCII chars
  constexpr int extra_chars = 9;  // Extra chars we've rempped that aren't just an ASCII char plus an accent
  ...
  struct font_t {
      ...
      const uint8_t max_width;
      const uint8_t widths[base_chars + extra_chars];
      const uint8_t data[3*1024];  // ALlocate space for worse case font size
  };
#endif
