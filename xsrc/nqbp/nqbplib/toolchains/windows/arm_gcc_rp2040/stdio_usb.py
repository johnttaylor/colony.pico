...
class ToolChain( base.ToolChain ):
    ...
    def __init__( self, exename, prjdir, build_variants, board, pico_root, default_variant='release', env_error=None ):
        ...
        self._pad_chksum = os.path.join( pico_root, 'pico-sdk', 'src', 'rp2_common', 'boot_stage2', "pad_checksum.py" )
        self._elf2uf2    = os.path.join( pico_root, 'pico-sdk', 'tools', 'elf2uf2.exe' )
        self._pioasm     = os.path.join( pico_root, 'pico-sdk', 'tools', 'pioasm.exe' )
        self._asm_ext    = 'asm'    
        self._asm_ext2   = 'S'   

        self._clean_pkg_dirs.extend( ['pico-sdk', '_pico'] )

        # set the name of the linker output (not the final output)
        self._link_output = '-o ' + exename + '.elf'

        # Define paths
        sdk_src_path = os.path.join( pico_root, 'pico-sdk', 'src' )
        ...
        # USB support
        usb_src_path = os.path.join( pico_root, 'pico-sdk', 'lib', 'tinyusb' )
        self._base_release.inc = self._base_release.inc + \
        ...
        board_header = os.path.join( pico_root, "pico-sdk", "src", "boards", "include", "boards", board + '.h' )

          
       #
        mcu                             = '-mcpu=cortex-m0plus'
        common_flags                    = f' -O3 -DPICO_CONFIG_HEADER={board_header} {mcu} -mthumb -ffunction-sections -fdata-sections -Wno-array-bounds -DPICO_TARGET_NAME=\\"{exename}\\" -DPICO_BOARD=\\"{board}\\" -DLIB_PICO_BIT_OPS=1 -DLIB_PICO_BIT_OPS_PICO=1 -DLIB_PICO_DIVIDER=1 -DLIB_PICO_DIVIDER_HARDWARE=1 -DLIB_PICO_DOUBLE=1 -DLIB_PICO_DOUBLE_PICO=1 -DLIB_PICO_FLOAT=1 -DLIB_PICO_FLOAT_PICO=1 -DLIB_PICO_INT64_OPS=1 -DLIB_PICO_INT64_OPS_PICO=1 -DLIB_PICO_MALLOC=1 -DLIB_PICO_MEM_OPS=1 -DLIB_PICO_MEM_OPS_PICO=1 -DLIB_PICO_PLATFORM=1 -DLIB_PICO_PRINTF=1 -DLIB_PICO_PRINTF_PICO=1 -DLIB_PICO_RUNTIME=1 -DLIB_PICO_STANDARD_LINK=1 -DLIB_PICO_STDIO=1 -DLIB_PICO_STDLIB=1 -DLIB_PICO_SYNC=1 -DLIB_PICO_SYNC_CORE=1 -DLIB_PICO_SYNC_CRITICAL_SECTION=1 -DLIB_PICO_SYNC_MUTEX=1 -DLIB_PICO_SYNC_SEM=1 -DLIB_PICO_TIME=1 -DLIB_PICO_UTIL=1 -DPICO_BUILD=1 -DLIB_PICO_UNIQUE_ID=1'
        ...
        self._base_release.linkflags    = f' -fno-rtti -fno-exceptions -fno-unwind-tables -fno-use-cxa-atexit {mcu} -mthumb -Wl,--build-id=none --specs=nosys.specs -Wl,-z,max-page-size=4096 -Wl,--gc-sections {wrapper_funcs} -Wl,--script={linker_script} bs2_default_padded_checksummed.S -Wl,-Map={exename}.elf.map' 
        ...
        self._bootloader_link_flags     = f'-nostartfiles {mcu} -O3 -DNDEBUG -Wl,--build-id=none --specs=nosys.specs -nostartfiles -Wl,--script={boot_linker_script} -Wl,-Map=bs2_default.elf.map {boot_obj} -o bs2_default.elf'   
        self._base_release.linklibs     = ''

        # 
        self._base_release.firstobjs    = r'_BUILT_DIR_.pico-sdk\src\rp2_common\pico_standard_link'
