...
class ToolChain( base.ToolChain ):
   ...
   #--------------------------------------------------------------------------
    def link( self, arguments, inf, local_external_setting, variant ):
        ...
        utils.pop_dir()
        try:
            shutil.copy(self._path_cy43_firmware, ".")
        except Exception as ex:
            self._printer.output("=")
            self._printer.output(f"= Build Failed: Failed find {self._path_cy43_firmware} ({ex})" )
            self._printer.output("=")
            sys.exit(1)
        self._printer.output("= Creating CYW43 Firmware object file..." )
        fwblob  = self._cyw43_firmware_bin.replace('.','_').replace('-','_')
        fwblob2 = 'fw_' + fwblob[:fwblob.rfind('_')]
        objcpy = f'{self._objcpy} -I binary -O elf32-littlearm -B arm --readonly-text --rename-section .data=.big_const,contents,alloc,load,readonly,data'
        ...
        objcpy = f'{objcpy} {self._cyw43_firmware_bin} cyw43_resource.o'
        if ( arguments['-v'] ):
            self._printer.output( objcpy )
        if ( utils.run_shell(self._printer, objcpy) ):
            self._printer.output("=")
            self._printer.output("= Build Failed: Failed to create CYW43 Firmware object file" )
            self._printer.output("=")
            sys.exit(1)
        utils.delete_file(self._cyw43_firmware_bin)
