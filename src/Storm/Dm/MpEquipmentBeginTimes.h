...
public:
    ...
    {
        return readData( &dstData, sizeof( Storm::Type::EquipmentTimes_T ), seqNumPtr );
    }

    /// Type safe write. See Cpl::Dm::ModelPoint
    inline uint16_t write( const Storm::Type::EquipmentTimes_T& srcData, LockRequest_T lockRequest = eNO_REQUEST ) noexcept
    {
        return writeData( &srcData, sizeof( Storm::Type::EquipmentTimes_T ), lockRequest );
    }
...
public:
    /** This convenience method is used to read the MP contents and synchronize
        ...
        Note: The observer will be subscribed for change notifications after
              this call.
     */
    inline bool readAndSync( Storm::Type::EquipmentTimes_T& dst, Observer& observerToSync )
    {
        uint16_t seqNum;
